...
function save_options() {
  ...
  options.autoSwissIdCode = document.getElementById("chkSwissIdCode").checked;
...
function restore_options() {
  ...
  chrome.storage.sync.get(
    ...
    function(items) {
      ...
      document.getElementById("chkSwissIdCode").checked = items.autoSwissIdCode;
      checkPwVisibility();
...
document
  .getElementById("chkFillPassword")
  .addEventListener("click", checkPwVisibility);

//for test puropses
if (!chrome.storage) {
  chrome.storage = {
    sync: {
      get: function(item, fn) {
        fn(item);
      },
      set: function(item, fn) {
        fn(item);
      }
    }
  };
